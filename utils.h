...
extern int	ix[8];
extern int	iy[8];
extern int index[8][3];   //write by Wei
...
inline int Get_rowToPlus(int dir, int row,int N)
{
	return( row + ix[index[N][dir]] );
}
inline int Get_colToPlus(int dir, int col,int N){
	return( col + iy[index[N][dir]] );
}
